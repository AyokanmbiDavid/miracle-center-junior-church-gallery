  ...
  return (
    <div className="w-full flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 pb-4 border-b border-stone-200 dark:border-zinc-900">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <nav className="flex gap-1 p-1 bg-white dark:bg-zinc-900 rounded-full border border-stone-200 dark:border-zinc-800">
          {TABS.map((tab) => {
            const Icon = tab.icon;
            const isActive = activeTab === tab.id;
            return (
              <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className="relative px-3 p-3 bg-transparent border-0 cursor-pointer outline-none flex items-center gap-2 rounded-full text-xs font-medium transition-colors">
                {isActive && ( <motion.div layoutId="subActiveTab" className="absolute inset-0 bg-stone-100 dark:bg-zinc-800 rounded-full border border-stone-200/60 dark:border-zinc-700/60 shadow-xs z-0" transition={{ type: "spring", stiffness: 450, damping: 32 }} /> )}
        ...
        <div className="relative w-full sm:w-56">
          ...
          <input type="search" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={`Filter ${activeTab}...`} className="w-full pl-9 pr-4 py-3 text-xs bg-white dark:bg-zinc-900 border border-stone-200 dark:border-zinc-800 rounded-full text-stone-800 dark:text-zinc-100 placeholder-stone-400 dark:placeholder-zinc-600 focus:outline-none" />
        </div>
      </div>
    </div>
  );

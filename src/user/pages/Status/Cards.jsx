...
import { Image, Heart, ShareNetwork, Sun, Moon, DownloadSimpleIcon} from "@phosphor-icons/react";
...
export default function CardGrid({setrefreshf}) {
  ...
  return (
    <div >
      <div className="w-full min-h-screen overflow-y-auto bg-gray-50 dark:bg-gray-900 p-6 text-gray-900 dark:text-white transition-colors duration-300">

        {/* Responsive Grid */}
        <div className="grid mb-10 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 max-w-7xl mx-auto">
          <AnimatePresence mode="wait">
            {loading
              ...
              : Array.from({ length: CARDS_COUNT }).map((_, index) => (
                  <ActualCard key={`card-${index}`} id={index + 1} />
                ))}
          </AnimatePresence>
        </div>
      </div>

...
export default function MemberEditModal({ isOpen, onClose, initialData = null, onSave }) {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({ name: "", className: "Toddlers", role: "Member", imageFile: null });
      setExistingImageUrl("");
    }
  }, [initialData, isOpen]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const memberId = initialData ? initialData._id || initialData.id : null;
      await onSave(memberId, submitData);
      onClose();
    } catch (err) {
      console.error("Error saving member form:", err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-sm bg-white dark:bg-zinc-900 border border-stone-200 dark:border-zinc-800 rounded-xl p-5 shadow-xl text-stone-800 dark:text-zinc-100 animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex justify-between items-center mb-5 border-b border-stone-100 dark:border-zinc-800 pb-3">
          <h2 className="text-xs font-mono font-bold uppercase tracking-widest text-amber-700 dark:text-amber-500 flex items-center gap-1.5">
            ...
            {initialData ? "Modify Member Record" : "Enroll Member to Fold"}
          </h2>
          <button
            onClick={onClose}
            type="button"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 text-xs font-medium">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              className="w-full p-3 bg-stone-50 dark:bg-zinc-950 border border-stone-200 dark:border-zinc-800 rounded-full text-stone-800 dark:text-zinc-100 placeholder-stone-400 dark:placeholder-zinc-600 focus:outline-none focus:border-amber-600 dark:focus:border-amber-500"
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-stone-500 dark:text-zinc-400 font-mono text-[10px] uppercase tracking-wider flex items-center gap-1">
              <UsersThree size={12} /> Class Cohort
            </label>
            <select
              value={formData.className}
              onChange={(e) => setFormData({ ...formData, className: e.target.value })}
              className="w-full p-3 bg-stone-50 dark:bg-zinc-950 border border-stone-200 dark:border-zinc-800 rounded-full text-stone-800 dark:text-zinc-100 focus:outline-none focus:border-amber-600 dark:focus:border-amber-500 font-mono text-[11px]"
            >
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-stone-500 dark:text-zinc-400 font-mono text-[10px] uppercase tracking-wider flex items-center gap-1">
              <GraduationCap size={12} /> Ministry Role
            </label>
            <select
              value={formData.role}
          ...
          <div className="flex justify-end gap-2 mt-4 pt-4 border-t border-stone-100 dark:border-zinc-800">
            <button
              ...
              onClick={onClose}
              disabled={loading}
              className="px-4 py-3 text-xs font-mono font-medium bg-stone-100 hover:bg-stone-200 dark:bg-zinc-950 dark:hover:bg-zinc-900 border border-stone-200 dark:border-zinc-800 text-stone-600 dark:text-zinc-400 rounded-full transition-colors cursor-pointer disabled:opacity-50"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="px-4 py-2 flex items-center justify-center gap-1.5 bg-amber-600 hover:bg-amber-700 dark:bg-amber-600 dark:hover:bg-amber-700 text-white rounded-full font-mono font-bold shadow-md shadow-amber-700/10 transition-colors cursor-pointer disabled:bg-stone-400 dark:disabled:bg-zinc-800"
            >
              {loading ? (
                <>
                  <span>Saving...</span>
